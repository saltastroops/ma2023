...
class JWT {
    ...
    public static function decode($token, $key, array $allowed_algs = array())
    {
        ...
        if (empty($key)) {
            ...
            $payload->verify = 0;
            $payload->reason = 'Invalid or missing key';
        }
        ...
        if (count($tokenSegments) != 3) {
            ...
            $payload->verify = 0;
            $payload->reason = 'Wrong number of segments';
        }
        ...
        if (null === ($header = static::jsonDecode(static::urlsafeB64Decode($headb64)))) {
            ...
            $payload->verify = 0;
            $payload->reason = 'Invalid header encoding';
        }

        if (null === $payload = static::jsonDecode(static::urlsafeB64Decode($bodyb64))) {
            //throw new Exception('Invalid claims encoding');
            $payload->verify = 0;
            $payload->reason = 'Invalid claims encoding';
        }

        if (false === ($sig = static::urlsafeB64Decode($cryptob64))) {
            //throw new Exception('Invalid signature encoding');
            $payload->verify = 0;
            $payload->reason = 'Invalid signature encoding';
        }

        if (empty($header->alg)) {
            //throw new Exception('Empty algorithm');
            $payload->verify = 0;
            $payload->reason = 'Empty algorithm';
        }

        if (empty(static::$supported_algs[$header->alg])) {
            //throw new Exception('Algorithm not supported');
            $payload->verify = 0;
            $payload->reason = 'Algorithm not supported';
        }

        if (!in_array($header->alg, $allowed_algs)) {
            //throw new Exception('Algorithm not allowed');
            $payload->verify = 0;
            $payload->reason = 'Algorithm not allowed';
        }
        ...
        if (is_array($key) || $key instanceof ArrayAccess) {
            if (isset($header->kid)) {
                if (!isset($key[$header->kid])) {
                    ...
                    $payload->verify = 0;
                    $payload->reason = '"kid" invalid, unable to lookup correct key';
                }
                $key = $key[$header->kid];
            } else {
                //throw new UnexpectedValueException('"kid" empty, unable to lookup correct key');
                $payload->verify = 0;
                $payload->reason = '"kid" empty, unable to lookup correct key';
            }
        }
        ...
        if (!static::verify("$headb64.$bodyb64", $sig, $key, $header->alg)) {
            ...
            $payload->verify = 0;
            $payload->reason = 'Signature verification failed';
        } else {
            $payload->verify = 1;
        }
        ...
        if (isset($payload->nbf) && $payload->nbf > ($timestamp + static::$leeway)) {
            ...
            // );
            $payload->verify = 0;
            $payload->reason = 'Cannot handle token prior to ' . date(DateTime::ISO8601, $payload->nbf);
        }
        ...
        if (isset($payload->iat) && $payload->iat > ($timestamp + static::$leeway)) {
            ...
            // );
            $payload->verify = 0;
            $payload->reason = 'Cannot handle token prior to ' . date(DateTime::ISO8601, $payload->iat);
        }
